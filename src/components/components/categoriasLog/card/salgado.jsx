...
const Salgados = () => {
  const [lanches, setSalgados] = useState([]);
  const [selectedId, setSelectedId] = useState(null);

  useEffect(() => {
    axios.get('https://funny-handkerchief-newt.cyclic.app/buscar/salgados')
      .then(response => {
      ...
      {selectedId && (
        <BtnDeletar
          onDeletar={() => {
            ...
            setSalgados(prevLanches => prevLanches.filter(item => item.id !== selectedId));
            setSelectedId(null);
          }}
